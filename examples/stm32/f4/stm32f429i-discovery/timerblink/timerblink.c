...
static void tim_setup(void)
{
  /* Enable TIM1 clock. */
	...
	timer_set_oc_value(TIM1, TIM_OC1, 659); // no usar los negativos
...
int main(void)
{
  ...
  tim_setup();
    ...
    timer_set_oc_value(TIM1, TIM_OC1, 659);
		for (i = 0; i < 2*n; i++) {
			__asm__("nop");
		}

    timer_set_oc_value(TIM1, TIM_OC1, 790);
		for (i = 0; i < n; i++) {
			__asm__("nop");
		}    

    timer_set_oc_value(TIM1, TIM_OC1, 1317);
		for (i = 0; i < 3*n; i++) {
			__asm__("nop");
		}    

    timer_set_oc_value(TIM1, TIM_OC1, 988);
		for (i = 0; i < n; i++) {
			__asm__("nop");
		}
